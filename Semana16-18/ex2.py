...
#Declarando função de transferência
def x_pt(t,x,u):
...
def rk4(tk,h,xk,uk):
    #Transforma a matriz em uma sequência linear
    xk = xk.reshape([2,1])
    uk = uk.reshape([1,1])
    k1 = x_pt(tk,xk,uk)
    k2 = x_pt(tk+h/2.0,xk+h*k1/2.0,uk)
    k3 = x_pt(tk+h/2.0,xk+h*k2/2.0,uk)
    k4 = x_pt(tk+h,xk+h*k3,uk)
...
if __name__ == '__main__':
    ...
    # Parâmetro de um compensador em avanço com MS <= 10% ts = 2s
    MS = 0.10
    ts = 2
    ...
    # plt.plot(tout2,yout2,label='Clead*G')
    # plt.grid()
    # plt.legend()
    # plt.show()
